...
namespace ukive {
    ...
    void AssistConfigure::init() {
        ...
        assist_vs_.reset(gpu_device->createVertexShader(vs_bc.data(), vs_bc.size()));

        std::string ps_bc;
        res_mgr->getFileData(shader_dir / u"assist_pixel_shader.cso", &ps_bc);
        assist_ps_.reset(gpu_device->createPixelShader(ps_bc.data(), ps_bc.size()));
        ...
        desc[2] = GILDesc("TEXCOORD", 0, 0, GPUInputLayout::Append, GPUDataFormat::R32G32_FLOAT);
        assist_il_.reset(gpu_device->createInputLayout(desc, 3, vs_bc.data(), vs_bc.size()));
        ...
        assist_cb_.reset(gpu_device->createBuffer(&buf_desc, nullptr));
    }
        ...
        GPUContext* context, const ukv3d::Matrix4x4F& matrix)
    {
        ...
        context->setVConstantBuffers(0, 1, assist_cb_.get());
    }
